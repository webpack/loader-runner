...
function utf8BufferToString(buf) {
	...
	return str.charCodeAt(0) === BOM ? str.slice(1) : str;
}

/**
 * Escape `#` characters with a preceding `\0` byte. Short-circuits when the
 * input contains no `#`, avoiding the regex scan for the common case.
...
function iterateNormalLoaders(options, loaderContext, args, callback) {
	// Iterative walk: skip over executed loaders and those lacking a normal fn
	// without recursing, which avoids deep stacks for chains of sync loaders.
...
/**
 * Join loader requests into a single `!`-separated string for a range of
 * loader indices. Appends the resource (or "") to mirror the original
 * `map().concat(resource || "").join("!")` semantics while avoiding the
 * intermediate arrays.
...
module.exports.runLoaders = function runLoaders(options, callback) {
	...
	Object.defineProperty(loaderContext, "remainingRequest", {
		enumerable: true,
		get() {
			const resourceStr = loaderContext.resource || "";
			if (loaderContext.loaderIndex >= loaders.length - 1 && !resourceStr) {
				return "";
			}
			return joinRequests(
				loaders,
